 ...
 *
 * 功能：
 * - 直接用系统时钟计数接收空闲高电平的 8N1 UART 字节，不包含字节缓存、
 *   协议组帧或 CRC 处理。
 *
 * 关键约束：
 * - 每个位固定使用 SYS_CLK_FREQ/BAUD_RATE 个时钟周期，除法余数被截断，
 *   不进行分数波特率补偿；参数必须保证三个采样点合法且计数器不溢出。
 *
 * 特殊行为：
 * - 起始位、数据位和停止位均在位中心附近进行三点多数表决。
 * - 无效起始位视为毛刺并静默丢弃；停止位为低时报告帧错误而不提交字节。
 * - 停止位完成表决后提前退出接收，以减小背靠背字节的相位累积误差。
 */
